...
class UnlabeledPoseGenerator(Dataset):
    def __init__(self, poses_2d, cameras, actions, poses_3d):
        ...
        self._actions = reduce(lambda x, y: x + y, actions)
        self._cameras = []
        for seq_id in range(len(poses_2d)):
            for i in range(len(poses_2d[seq_id])):
                self._cameras.append(cameras[seq_id])
        assert self._poses_2d.shape[0] == len(self._actions)
        print('Generating {} poses...'.format(len(self._actions)))

    def __getitem__(self, index):
        out_pose_2d = self._poses_2d[index]
        out_pose_3d = self._poses_3d[index]
        out_camera = self._cameras[index]
        out_pose_2d = torch.from_numpy(out_pose_2d).float()

        return out_pose_2d, out_pose_3d, out_camera
